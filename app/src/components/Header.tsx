...
export default function Header() {
  ...
  return (
    <header className="bg-white border-b border-[var(--border-color)] px-6 md:px-8 py-4 flex items-center justify-between">
      <div>
        <div className="text-[11px] font-bold uppercase tracking-[0.15em] text-[var(--accent-primary)] font-mono">
          {meta.breadcrumb}
        </div>
        <div className="mt-0.5 text-xl md:text-2xl font-extrabold text-[var(--text-main)]">{meta.title}</div>
      </div>
      <div className="flex items-center gap-4">
        <div className="hidden sm:block text-sm text-[var(--text-muted)] capitalize">{today}</div>
        <div className="flex items-center gap-2.5 pl-4 border-l border-[var(--border-color)]">
          {anhUrl ? (
